import math
import re
import io
import os
import base64
import pandas as pd
from io import StringIO
from datetime import datetime, date
import traceback
import streamlit as st
from openpyxl import Workbook
from openpyxl.styles import (
    PatternFill, Font, Alignment, Border, Side, GradientFill
)
from openpyxl.utils import get_column_letter

# ==============================
# VERSÃO
# ==============================
VERSAO = "V1.3"

# ==============================
# TEMA TR (idêntico ao RPA V3.9)
# ==============================
def apply_tr_theme():
    st.markdown("""
        <style>
        html, body, [class*="css"] {
            font-family: 'Segoe UI', 'Arial', sans-serif;
            color: #444444;
        }
        h1, h2, h3 {
            color: #FF8000;
            font-weight: 700;
        }
        section[data-testid="stSidebar"] {
            background-color: #444444;
            color: #FFFFFF;
        }
        section[data-testid="stSidebar"] * {
            color: #FFFFFF !important;
        }
        .stButton > button {
            background-color: #FF8000;
            color: #FFFFFF;
            border: none;
            border-radius: 4px;
            font-weight: bold;
        }
        .stButton > button:hover {
            background-color: #D64001;
            color: #FFFFFF;
        }
        .stDownloadButton > button {
            background-color: #FF8000;
            color: #FFFFFF;
            border: none;
            border-radius: 4px;
            font-weight: bold;
        }
        .stDownloadButton > button:hover {
            background-color: #D64001;
            color: #FFFFFF;
        }
        hr {
            border-color: #FF8000;
        }
        [data-testid="metric-container"] {
            background-color: #E9E9E9;
            border-left: 4px solid #FF8000;
            border-radius: 4px;
            padding: 10px;
        }
        .instrucoes-box {
            background-color: #E9E9E9;
            border-left: 4px solid #FF8000;
            border-radius: 4px;
            padding: 16px 20px;
            margin: 12px 0;
            color: #444444;
            font-family: 'Segoe UI', Arial, sans-serif;
        }
        .instrucoes-box h4 {
            color: #FF8000;
            margin-top: 14px;
            margin-bottom: 6px;
        }
        .instrucoes-box h4:first-child {
            margin-top: 0;
        }
        </style>
    """, unsafe_allow_html=True)


# ==============================
# DECODE COM FALLBACK
# ==============================
def decode_arquivo(raw: bytes) -> str:
    for enc in ('utf-8', 'latin-1', 'cp1252'):
        try:
            return raw.decode(enc)
        except UnicodeDecodeError:
            continue
    return raw.decode('utf-8', errors='replace')


# ==============================
# ENCODE ANSI COM LOG
# ==============================
def encode_ansi_seguro(conteudo: str, log: list) -> bytes:
    resultado     = []
    substituicoes = 0
    for char in conteudo:
        try:
            resultado.append(char.encode('latin-1'))
        except UnicodeEncodeError:
            resultado.append(b'?')
            substituicoes += 1
    if substituicoes:
        log.append(
            f"AVISO: {substituicoes} caractere(s) fora do padrão ANSI "
            f"substituídos por '?'."
        )
    return b''.join(resultado)


# ==============================
# PARSER DO ARQUIVO DOMÍNIO
# ==============================
def parse_dominio(content: str) -> dict:
    """
    Lê o arquivo Domínio Sistemas e agrupa as linhas por tipo de registro,
    preservando a ordem original para análise de hierarquia.
    """
    linhas_ordenadas = []
    por_tipo         = {}

    for num_linha, linha in enumerate(content.splitlines(), start=1):
        linha = linha.strip()
        if not linha:
            continue
        campos = linha.split('|')
        if campos and campos[0] == '':
            campos = campos[1:]
        if campos and campos[-1] == '':
            campos = campos[:-1]
        if len(campos) < 1:
            continue
        tipo = campos[0].strip()
        if not tipo:
            continue
        linhas_ordenadas.append((tipo, campos, num_linha))
        if tipo not in por_tipo:
            por_tipo[tipo] = []
        por_tipo[tipo].append((campos, num_linha))

    return {
        'linhas_ordenadas': linhas_ordenadas,
        'por_tipo':         por_tipo,
    }


# ==============================
# ÍNDICES DE CAMPOS (baseado no exemplo fornecido)
# ==============================
IDX_0000_CNPJ       = 1
IDX_1000_NUM_NF     = 1
IDX_1000_CNPJ_EMIT  = 2
IDX_1000_CFOP       = 4
IDX_1000_DT_EMISSAO = 11
IDX_1000_DT_ENTRADA = 12
IDX_1000_VL_TOTAL   = 13
IDX_1020_NUM_NF     = 1
IDX_1020_VL_TOTAL   = 3
IDX_1020_ALIQ_ICMS  = 4
IDX_1020_VL_ICMS    = 5
IDX_1030_NUM_ITEM   = 1
IDX_1030_QTD        = 2
IDX_1030_VL_UNIT    = 3
IDX_1030_DT_DOC     = 7
IDX_1030_VL_ITEM    = 10
IDX_1030_VL_DESC    = 11
IDX_1030_ALIQ_ICMS  = 14
IDX_1030_VL_ICMS    = 21
IDX_1030_CFOP       = 34
IDX_1300_DATA       = 1
IDX_1300_COD_HIST   = 2
IDX_1300_NUM_HIST   = 3
IDX_1300_VL         = 4
IDX_1300_NUM_NF     = 5
IDX_1300_HIST       = 6


def _campo(campos: list, idx: int, default: str = '') -> str:
    return campos[idx].strip() if len(campos) > idx else default


# ==============================
# EXTRAÇÃO DE CFOPs DO ARQUIVO
# ==============================
def extrair_cfops_do_arquivo(parsed: dict) -> dict:
    """
    Extrai todos os CFOPs únicos encontrados no arquivo,
    identificando em quais registros aparecem e quantas vezes.
    Retorna:
        {
          cfop: {
            'registros': set de tipos onde aparece,
            'ocorrencias': int,
            'tipo_operacao': 'Entrada' | 'Saída' | 'Desconhecido'
          }
        }
    """
    cfops = {}

    def registrar(cfop_raw: str, tipo_reg: str):
        cfop = str(cfop_raw).strip().zfill(4)
        # Filtra valores claramente inválidos
        if not cfop or cfop in ('0000', 'CFOP', 'nan', 'NAN') or not cfop.isdigit():
            return
        if cfop not in cfops:
            primeiro_dig = cfop[0]
            if primeiro_dig in ('1', '2', '3'):
                tipo_op = 'Entrada'
            elif primeiro_dig in ('5', '6', '7'):
                tipo_op = 'Saída'
            else:
                tipo_op = 'Desconhecido'
            cfops[cfop] = {
                'registros':     set(),
                'ocorrencias':   0,
                'tipo_operacao': tipo_op,
            }
        cfops[cfop]['registros'].add(tipo_reg)
        cfops[cfop]['ocorrencias'] += 1

    for tipo, campos, _ in parsed['linhas_ordenadas']:
        if tipo == '1000':
            cfop_val = _campo(campos, IDX_1000_CFOP)
            if cfop_val:
                registrar(cfop_val, '1000')
        elif tipo == '1030':
            cfop_val = _campo(campos, IDX_1030_CFOP)
            if cfop_val:
                registrar(cfop_val, '1030')

    return cfops


# ==============================
# DESCRIÇÕES DE CFOP (resumidas)
# ==============================
DESCRICOES_CFOP = {
    # ENTRADAS
    '1101': 'Compra p/ industrialização - dentro do estado',
    '1102': 'Compra p/ comercialização - dentro do estado',
    '1111': 'Compra p/ industrialização em zona franca',
    '1113': 'Compra p/ comercialização em zona franca',
    '1116': 'Compra p/ industrialização - devol. de remessa',
    '1120': 'Compra p/ industrialização - sem transitar pelo estabelecimento',
    '1201': 'Devolução de venda dentro do estado',
    '1202': 'Devolução de venda dentro do estado - tributada',
    '1203': 'Devolução de venda dentro do estado - não tributada',
    '1251': 'Compra p/ ativo imobilizado dentro do estado',
    '1252': 'Compra p/ ativo imobilizado fora do estado',
    '1301': 'Aquisição de serviço de transporte dentro do estado',
    '1351': 'Aquisição de serviço de comunicação dentro do estado',
    '1352': 'Aquisição de serviço de comunicação fora do estado',
    '1401': 'Compra p/ uso e consumo dentro do estado',
    '1403': 'Compra p/ uso e consumo fora do estado',
    '1501': 'Entrada de mercadoria recebida com fim específico de exportação',
    '1502': 'Entrada de mercadoria recebida com fim específico de exportação - NF',
    '1503': 'Entrada de mercadoria recebida com fim específico de exportação - conhecimento',
    '1601': 'Recebimento, por transferência, de crédito de ICMS',
    '1701': 'Entrada de mercadoria recebida em consignação mercantil',
    '1801': 'Entradas de mercadorias recebidas para industrialização por encomenda',
    '1901': 'Outras entradas de mercadorias não especificadas',
    '2101': 'Compra p/ industrialização - fora do estado',
    '2102': 'Compra p/ comercialização - fora do estado',
    '2111': 'Compra p/ industrialização em zona franca - fora do estado',
    '2113': 'Compra p/ comercialização em zona franca - fora do estado',
    '2116': 'Compra p/ industrialização - devol. de remessa - fora do estado',
    '2120': 'Compra p/ industrialização sem transitar - fora do estado',
    '2201': 'Devolução de venda fora do estado',
    '2202': 'Devolução de venda fora do estado - tributada',
    '2203': 'Devolução de venda fora do estado - não tributada',
    '2251': 'Compra p/ ativo imobilizado fora do estado',
    '2252': 'Compra p/ ativo imobilizado - importação',
    '2301': 'Aquisição de serviço de transporte fora do estado',
    '2351': 'Aquisição de serviço de comunicação fora do estado',
    '2352': 'Aquisição de serviço de comunicação - exterior',
    '2401': 'Compra p/ uso e consumo fora do estado',
    '2403': 'Compra p/ uso e consumo - importação',
    '2501': 'Entrada de mercadoria recebida com fim de exportação - fora do estado',
    '2502': 'Entrada de mercadoria recebida com fim de exportação - fora do estado NF',
    '2503': 'Entrada de mercadoria recebida com fim de exportação - fora do estado CT',
    '2601': 'Recebimento, por transferência, de crédito de ICMS - fora do estado',
    '2701': 'Entrada de mercadoria recebida em consignação - fora do estado',
    '2801': 'Entradas de mercadorias para industrialização por encomenda - fora do estado',
    '2901': 'Outras entradas de mercadorias não especificadas - fora do estado',
    '3101': 'Compra p/ industrialização - exterior',
    '3102': 'Compra p/ comercialização - exterior',
    # SAÍDAS
    '5101': 'Venda de produção do estabelecimento - dentro do estado',
    '5102': 'Venda de mercadoria adquirida ou recebida - dentro do estado',
    '5111': 'Venda de produção - zona franca - dentro do estado',
    '5113': 'Venda de mercadoria - zona franca - dentro do estado',
    '5151': 'Transferência de produção - dentro do estado',
    '5152': 'Transferência de mercadoria - dentro do estado',
    '5153': 'Transferência de energia elétrica - dentro do estado',
    '5201': 'Devolução de compra p/ industrialização - dentro do estado',
    '5202': 'Devolução de compra p/ comercialização - dentro do estado',
    '5251': 'Venda de ativo imobilizado - dentro do estado',
    '5301': 'Prestação de serviço de transporte - dentro do estado',
    '5351': 'Prestação de serviço de comunicação - dentro do estado',
    '5352': 'Prestação de serviço de comunicação - fora do estado',
    '5401': 'Venda de produção - substituição tributária - dentro do estado',
    '5403': 'Venda de mercadoria - substituição tributária - dentro do estado',
    '5501': 'Remessa de mercadoria p/ formação de lote de exportação',
    '5601': 'Transferência de crédito de ICMS - dentro do estado',
    '5701': 'Venda de mercadoria em consignação mercantil - dentro do estado',
    '5801': 'Remessa p/ industrialização por encomenda - dentro do estado',
    '5901': 'Outras saídas de mercadorias não especificadas',
    '6101': 'Venda de produção do estabelecimento - fora do estado',
    '6102': 'Venda de mercadoria adquirida ou recebida - fora do estado',
    '6111': 'Venda de produção - zona franca - fora do estado',
    '6113': 'Venda de mercadoria - zona franca - fora do estado',
    '6151': 'Transferência de produção - fora do estado',
    '6152': 'Transferência de mercadoria - fora do estado',
    '6153': 'Transferência de energia elétrica - fora do estado',
    '6201': 'Devolução de compra p/ industrialização - fora do estado',
    '6202': 'Devolução de compra p/ comercialização - fora do estado',
    '6251': 'Venda de ativo imobilizado - fora do estado',
    '6301': 'Prestação de serviço de transporte - fora do estado',
    '6351': 'Prestação de serviço de comunicação - fora do estado',
    '6352': 'Prestação de serviço de comunicação - exterior',
    '6401': 'Venda de produção - substituição tributária - fora do estado',
    '6403': 'Venda de mercadoria - substituição tributária - fora do estado',
    '6501': 'Remessa de mercadoria p/ formação de lote de exportação - fora do estado',
    '6601': 'Transferência de crédito de ICMS - fora do estado',
    '6701': 'Venda de mercadoria em consignação - fora do estado',
    '6801': 'Remessa p/ industrialização por encomenda - fora do estado',
    '6901': 'Outras saídas de mercadorias não especificadas - fora do estado',
}


# ==============================
# GERADOR DO XLSX DE ACUMULADORES — TEMA TR
# ==============================
def gerar_xlsx_acumuladores_tr(cfops_dict: dict) -> bytes:
    """
    Gera um arquivo XLSX com o tema Thomson Reuters contendo:
    - Aba 'Acumuladores': tabela CFOP → Acumulador para preenchimento
    - Aba 'CFOPs Encontrados': relatório dos CFOPs identificados no arquivo
    Retorna os bytes do arquivo .xlsx
    """
    wb = Workbook()

    # ── Cores TR ────────────────────────────────────────────────────────
    COR_LARANJA   = "FF8000"
    COR_CINZA_ESC = "444444"
    COR_CINZA_CLR = "E9E9E9"
    COR_BRANCO    = "FFFFFF"
    COR_LARANJA_C = "FFF3E0"   # laranja muito claro para linhas alternadas
    COR_VERDE_CLR = "E8F5E9"   # verde claro para CFOPs mapeados
    COR_VERM_CLR  = "FFEBEE"   # vermelho claro para CFOPs sem mapeamento

    borda_fina = Border(
        left=Side(style='thin',   color="CCCCCC"),
        right=Side(style='thin',  color="CCCCCC"),
        top=Side(style='thin',    color="CCCCCC"),
        bottom=Side(style='thin', color="CCCCCC"),
    )
    borda_laranja = Border(
        left=Side(style='medium',   color=COR_LARANJA),
        right=Side(style='medium',  color=COR_LARANJA),
        top=Side(style='medium',    color=COR_LARANJA),
        bottom=Side(style='medium', color=COR_LARANJA),
    )

    def fill(hex_color: str) -> PatternFill:
        return PatternFill("solid", fgColor=hex_color)

    def header_font(branco=True) -> Font:
        return Font(
            name='Segoe UI', bold=True, size=11,
            color=COR_BRANCO if branco else COR_CINZA_ESC
        )

    def cell_font(bold=False, color=COR_CINZA_ESC, size=10) -> Font:
        return Font(name='Segoe UI', bold=bold, size=size, color=color)

    def center() -> Alignment:
        return Alignment(horizontal='center', vertical='center', wrap_text=False)

    def left_align() -> Alignment:
        return Alignment(horizontal='left', vertical='center', wrap_text=False)

    # ════════════════════════════════════════════════════════════════════
    # ABA 1 — Acumuladores (para preenchimento pelo usuário)
    # ════════════════════════════════════════════════════════════════════
    ws1 = wb.active
    ws1.title = "Acumuladores"
    ws1.sheet_view.showGridLines = False

    # Linha 1 — Banner TR
    ws1.merge_cells("A1:E1")
    ws1.row_dimensions[1].height = 36
    c = ws1["A1"]
    c.value          = "Thomson Reuters  |  Domínio Sistemas  —  Tabela de Acumuladores CFOP"
    c.fill           = fill(COR_CINZA_ESC)
    c.font           = Font(name='Segoe UI', bold=True, size=13, color=COR_LARANJA)
    c.alignment      = Alignment(horizontal='left', vertical='center')

    # Linha 2 — subtítulo
    ws1.merge_cells("A2:E2")
    ws1.row_dimensions[2].height = 20
    c2 = ws1["A2"]
    c2.value     = (
        f"Gerado automaticamente a partir dos CFOPs identificados no arquivo  |  "
        f"{len(cfops_dict)} CFOP(s) encontrado(s)  |  {datetime.now().strftime('%d/%m/%Y %H:%M')}"
    )
    c2.fill      = fill(COR_LARANJA)
    c2.font      = Font(name='Segoe UI', bold=False, size=9, color=COR_BRANCO)
    c2.alignment = Alignment(horizontal='left', vertical='center')

    # Linha 3 — instrução
    ws1.merge_cells("A3:E3")
    ws1.row_dimensions[3].height = 18
    c3 = ws1["A3"]
    c3.value     = (
        "⚠  Preencha a coluna ACUMULADOR para cada CFOP antes de fazer o upload no conversor."
    )
    c3.fill      = fill(COR_CINZA_CLR)
    c3.font      = Font(name='Segoe UI', bold=True, size=9, color=COR_CINZA_ESC)
    c3.alignment = Alignment(horizontal='left', vertical='center')

    # Linha 4 — espaço
    ws1.row_dimensions[4].height = 6

    # Linha 5 — Cabeçalho da tabela
    ws1.row_dimensions[5].height = 22
    cabecalhos = ['CFOP', 'DESCRIÇÃO', 'TIPO OPERAÇÃO', 'OCORRÊNCIAS', 'ACUMULADOR']
    col_widths  = [10,     48,          16,               14,            16]

    for col_idx, (cab, width) in enumerate(zip(cabecalhos, col_widths), start=1):
        col_letter = get_column_letter(col_idx)
        ws1.column_dimensions[col_letter].width = width
        cell = ws1.cell(row=5, column=col_idx, value=cab)
        cell.fill      = fill(COR_LARANJA)
        cell.font      = header_font(branco=True)
        cell.alignment = center()
        cell.border    = borda_fina

    # Dados — CFOPs ordenados: primeiro por tipo_operacao, depois por CFOP
    cfops_ordenados = sorted(
        cfops_dict.items(),
        key=lambda x: (0 if x[1]['tipo_operacao'] == 'Entrada' else 1, x[0])
    )

    linha_dados = 6
    for idx, (cfop, info) in enumerate(cfops_ordenados):
        ws1.row_dimensions[linha_dados].height = 18

        # Cor alternada por tipo de operação
        if info['tipo_operacao'] == 'Entrada':
            bg = COR_CINZA_CLR if idx % 2 == 0 else COR_BRANCO
        else:
            bg = COR_LARANJA_C if idx % 2 == 0 else COR_BRANCO

        descricao   = DESCRICOES_CFOP.get(cfop, '— preencha a descrição —')
        regs_str    = ', '.join(sorted(info['registros']))
        tipo_op     = info['tipo_operacao']
        ocorrencias = info['ocorrencias']

        valores = [cfop, descricao, tipo_op, ocorrencias, '']

        for col_idx, valor in enumerate(valores, start=1):
            cell = ws1.cell(row=linha_dados, column=col_idx, value=valor)
            cell.fill   = fill(bg)
            cell.border = borda_fina

            if col_idx == 1:   # CFOP — negrito centralizado
                cell.font      = cell_font(bold=True, color=COR_CINZA_ESC)
                cell.alignment = center()
            elif col_idx == 3: # Tipo Operação — colorido
                cor_tipo = "1B5E20" if tipo_op == 'Entrada' else "B71C1C"
                cell.font      = cell_font(bold=True, color=cor_tipo)
                cell.alignment = center()
            elif col_idx == 4: # Ocorrências — centralizado
                cell.font      = cell_font(bold=False)
                cell.alignment = center()
            elif col_idx == 5: # ACUMULADOR — destaque laranja para preenchimento
                cell.fill      = fill("FFF8F0")
                cell.font      = cell_font(bold=True, color=COR_LARANJA)
                cell.alignment = center()
                cell.border    = Border(
                    left=Side(style='medium', color=COR_LARANJA),
                    right=Side(style='medium', color=COR_LARANJA),
                    top=Side(style='thin',    color="CCCCCC"),
                    bottom=Side(style='thin', color="CCCCCC"),
                )
            else:
                cell.font      = cell_font()
                cell.alignment = left_align()

        linha_dados += 1

    # Linha de rodapé
    ws1.row_dimensions[linha_dados].height = 18
    ws1.merge_cells(f"A{linha_dados}:E{linha_dados}")
    c_rod = ws1.cell(row=linha_dados, column=1,
                     value="Thomson Reuters  |  Domínio Sistemas  |  Gerado automaticamente")
    c_rod.fill      = fill(COR_CINZA_ESC)
    c_rod.font      = Font(name='Segoe UI', size=8, color="888888")
    c_rod.alignment = Alignment(horizontal='right', vertical='center')

    # Congela painel abaixo do cabeçalho
    ws1.freeze_panes = "A6"

    # Filtro automático
    ws1.auto_filter.ref = f"A5:E{linha_dados - 1}"

    # ════════════════════════════════════════════════════════════════════
    # ABA 2 — CFOPs Encontrados (relatório de análise)
    # ════════════════════════════════════════════════════════════════════
    ws2 = wb.create_sheet(title="CFOPs Encontrados")
    ws2.sheet_view.showGridLines = False

    # Banner
    ws2.merge_cells("A1:F1")
    ws2.row_dimensions[1].height = 36
    c = ws2["A1"]
    c.value     = "Thomson Reuters  |  Domínio Sistemas  —  Relatório de CFOPs Identificados"
    c.fill      = fill(COR_CINZA_ESC)
    c.font      = Font(name='Segoe UI', bold=True, size=13, color=COR_LARANJA)
    c.alignment = Alignment(horizontal='left', vertical='center')

    ws2.merge_cells("A2:F2")
    ws2.row_dimensions[2].height = 20
    c2 = ws2["A2"]
    c2.value     = (
        f"Arquivo analisado em {datetime.now().strftime('%d/%m/%Y às %H:%M')}  |  "
        f"Total: {len(cfops_dict)} CFOP(s) único(s)  |  "
        f"Entradas: {sum(1 for v in cfops_dict.values() if v['tipo_operacao']=='Entrada')}  |  "
        f"Saídas: {sum(1 for v in cfops_dict.values() if v['tipo_operacao']=='Saída')}"
    )
    c2.fill      = fill(COR_LARANJA)
    c2.font      = Font(name='Segoe UI', bold=False, size=9, color=COR_BRANCO)
    c2.alignment = Alignment(horizontal='left', vertical='center')

    ws2.row_dimensions[3].height = 6

    # Cabeçalho
    ws2.row_dimensions[4].height = 22
    cab2   = ['CFOP', 'DESCRIÇÃO', 'TIPO OPERAÇÃO', 'REGISTROS', 'OCORRÊNCIAS', 'STATUS MAPEAMENTO']
    wids2  = [10,     48,          16,               14,           14,            22]

    for col_idx, (cab, width) in enumerate(zip(cab2, wids2), start=1):
        ws2.column_dimensions[get_column_letter(col_idx)].width = width
        cell = ws2.cell(row=4, column=col_idx, value=cab)
        cell.fill      = fill(COR_LARANJA)
        cell.font      = header_font(branco=True)
        cell.alignment = center()
        cell.border    = borda_fina

    linha2 = 5
    for idx, (cfop, info) in enumerate(cfops_ordenados):
        ws2.row_dimensions[linha2].height = 18

        descricao   = DESCRICOES_CFOP.get(cfop, '— CFOP não catalogado —')
        regs_str    = ', '.join(sorted(info['registros']))
        tipo_op     = info['tipo_operacao']
        ocorrencias = info['ocorrencias']
        mapeado     = cfop in DESCRICOES_CFOP
        status      = '✔ Catalogado' if mapeado else '✘ Não catalogado'
        bg_status   = COR_VERDE_CLR if mapeado else COR_VERM_CLR

        bg = COR_CINZA_CLR if idx % 2 == 0 else COR_BRANCO

        vals2 = [cfop, descricao, tipo_op, regs_str, ocorrencias, status]

        for col_idx, valor in enumerate(vals2, start=1):
            cell = ws2.cell(row=linha2, column=col_idx, value=valor)
            cell.border = borda_fina

            if col_idx == 6:
                cell.fill      = fill(bg_status[1:] if bg_status.startswith('#') else bg_status)
                cor_txt        = "1B5E20" if mapeado else "B71C1C"
                cell.font      = cell_font(bold=True, color=cor_txt)
                cell.alignment = center()
            elif col_idx == 1:
                cell.fill      = fill(bg)
                cell.font      = cell_font(bold=True)
                cell.alignment = center()
            elif col_idx == 3:
                cell.fill      = fill(bg)
                cor_tipo       = "1B5E20" if tipo_op == 'Entrada' else "B71C1C"
                cell.font      = cell_font(bold=True, color=cor_tipo)
                cell.alignment = center()
            elif col_idx in (4, 5):
                cell.fill      = fill(bg)
                cell.font      = cell_font()
                cell.alignment = center()
            else:
                cell.fill      = fill(bg)
                cell.font      = cell_font()
                cell.alignment = left_align()

        linha2 += 1

    # Rodapé
    ws2.row_dimensions[linha2].height = 18
    ws2.merge_cells(f"A{linha2}:F{linha2}")
    c_rod2 = ws2.cell(row=linha2, column=1,
                      value="Thomson Reuters  |  Domínio Sistemas  |  Gerado automaticamente")
    c_rod2.fill      = fill(COR_CINZA_ESC)
    c_rod2.font      = Font(name='Segoe UI', size=8, color="888888")
    c_rod2.alignment = Alignment(horizontal='right', vertical='center')

    ws2.freeze_panes = "A5"
    ws2.auto_filter.ref = f"A4:F{linha2 - 1}"

    # ── Salva e retorna bytes ────────────────────────────────────────────
    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    return buf.read()


# ==============================
# CARREGAMENTO DA TABELA DE ACUMULADORES
# ==============================
def carregar_acumuladores(arquivo_bytes: bytes, nome_arquivo: str, log: list) -> dict | None:
    try:
        ext = os.path.splitext(nome_arquivo)[1].lower()
        if ext in ('.xlsx', '.xls'):
            df = pd.read_excel(io.BytesIO(arquivo_bytes), dtype=str)
        else:
            raw_str = arquivo_bytes.decode('latin-1', errors='replace')
            sep     = ';' if raw_str.count(';') >= raw_str.count(',') else ','
            df      = pd.read_csv(io.StringIO(raw_str), sep=sep, dtype=str)

        df.columns = [str(c).strip().upper() for c in df.columns]

        if 'CFOP' not in df.columns or 'ACUMULADOR' not in df.columns:
            log.append(
                "ERRO: O arquivo de acumuladores deve conter as colunas "
                "'CFOP' e 'ACUMULADOR'."
            )
            return None

        tabela = {}
        erros  = 0
        for _, row in df.iterrows():
            cfop = str(row['CFOP']).strip().zfill(4)
            acum = str(row['ACUMULADOR']).strip()
            if not cfop or not acum or cfop.upper() == 'NAN' or acum.upper() == 'NAN':
                erros += 1
                continue
            # Só aceita linhas onde o acumulador foi preenchido
            if acum in ('', '0', 'nan', 'NAN', 'None'):
                continue
            tabela[cfop] = acum

        if not tabela:
            log.append(
                "ERRO: Nenhum par CFOP → Acumulador válido encontrado. "
                "Verifique se a coluna ACUMULADOR foi preenchida."
            )
            return None

        if erros:
            log.append(
                f"AVISO: {erros} linha(s) ignoradas por dados inválidos."
            )

        log.append(
            f"Tabela de acumuladores carregada: {len(tabela)} CFOPs mapeados."
        )
        return tabela

    except Exception as e:
        log.append(f"ERRO ao carregar arquivo de acumuladores: {e}")
        return None


def get_acumulador(cfop: str, tabela: dict, nao_mapeados: set) -> str:
    cfop_norm = str(cfop).strip().zfill(4)
    acum = tabela.get(cfop_norm)
    if acum is None:
        nao_mapeados.add(cfop_norm)
        return '9999'
    return acum


# ==============================
# HIERARQUIA E ORDENAÇÃO
# ==============================
def extrair_notas_com_itens(
    parsed: dict,
    tabela_acum: dict,
    nao_mapeados: set,
    log: list,
) -> list:
    linhas     = parsed['linhas_ordenadas']
    notas      = []
    nota_atual = None

    for tipo, campos, num_linha in linhas:
        if tipo == '0000':
            continue

        elif tipo == '1000':
            if nota_atual is not None:
                notas.append(nota_atual)
            num_nf     = _campo(campos, IDX_1000_NUM_NF)
            cnpj_emit  = _campo(campos, IDX_1000_CNPJ_EMIT)
            cfop_nf    = _campo(campos, IDX_1000_CFOP)
            dt_emissao = _campo(campos, IDX_1000_DT_EMISSAO)
            dt_entrada = _campo(campos, IDX_1000_DT_ENTRADA)
            vl_total   = _campo(campos, IDX_1000_VL_TOTAL)
            acum_nf    = get_acumulador(cfop_nf, tabela_acum, nao_mapeados)
            nota_atual = {
                'num_nf':      num_nf,
                'cnpj_emit':   cnpj_emit,
                'cfop_nf':     cfop_nf,
                'acum_nf':     acum_nf,
                'dt_emissao':  dt_emissao,
                'dt_entrada':  dt_entrada,
                'vl_total':    vl_total,
                'campos_1000': campos,
                'totais_1020': None,
                'itens_1030':  [],
                'lanc_1300':   [],
                'linha_orig':  num_linha,
            }

        elif tipo == '1020':
            if nota_atual is not None:
                nota_atual['totais_1020'] = campos
            else:
                log.append(
                    f"AVISO: Registro 1020 na linha {num_linha} sem 1000 pai. Ignorado."
                )

        elif tipo == '1030':
            if nota_atual is not None:
                num_item  = _campo(campos, IDX_1030_NUM_ITEM)
                cfop_item = _campo(campos, IDX_1030_CFOP)
                acum_item = get_acumulador(cfop_item, tabela_acum, nao_mapeados)
                nota_atual['itens_1030'].append({
                    'num_item':   num_item,
                    'cfop':       cfop_item,
                    'acum':       acum_item,
                    'campos':     campos,
                    'linha_orig': num_linha,
                })
            else:
                log.append(
                    f"AVISO: Registro 1030 na linha {num_linha} sem 1000 pai. Ignorado."
                )

        elif tipo == '1300':
            if nota_atual is not None:
                nota_atual['lanc_1300'].append(campos)
            else:
                log.append(
                    f"AVISO: Registro 1300 na linha {num_linha} sem 1000 pai. Ignorado."
                )

    if nota_atual is not None:
        notas.append(nota_atual)

    def sort_key_nota(n):
        dt_str = n['dt_emissao']
        try:
            dt = datetime.strptime(dt_str, '%d/%m/%Y')
        except Exception:
            try:
                dt = datetime.strptime(dt_str, '%Y-%m-%d')
            except Exception:
                dt = datetime.min
        try:
            num = int(n['num_nf'])
        except Exception:
            num = 0
        return (dt, num)

    notas.sort(key=sort_key_nota)

    for nota in notas:
        nota['itens_1030'].sort(
            key=lambda it: int(it['num_item']) if it['num_item'].isdigit() else 0
        )

    log.append(
        f"Hierarquia montada: {len(notas)} nota(s), "
        f"ordenadas por data de emissão e número."
    )
    return notas


# ==============================
# GERADOR DO ARQUIVO DE SAÍDA
# ==============================
def gerar_saida(parsed: dict, tabela_acum: dict, log: list) -> tuple:
    nao_mapeados = set()
    saida        = StringIO()
    stats = {
        'notas':       0,
        'nf_entrada':  0,
        'nf_saida':    0,
        'itens':       0,
        'lancamentos': 0,
        'erros':       0,
    }

    if '0000' in parsed['por_tipo']:
        campos_0000, _ = parsed['por_tipo']['0000'][0]
        saida.write('|' + '|'.join(campos_0000) + '|\n')
    else:
        log.append("AVISO: Registro 0000 não encontrado no arquivo.")

    notas = extrair_notas_com_itens(parsed, tabela_acum, nao_mapeados, log)

    for nota in notas:
        saida.write('|' + '|'.join(nota['campos_1000']) + '|\n')
        stats['notas'] += 1
        cfop_ini = nota['cfop_nf'][:1] if nota['cfop_nf'] else ''
        if cfop_ini in ('1', '2', '3'):
            stats['nf_entrada'] += 1
        elif cfop_ini in ('5', '6', '7'):
            stats['nf_saida'] += 1

        if nota['totais_1020'] is not None:
            saida.write('|' + '|'.join(nota['totais_1020']) + '|\n')

        for item in nota['itens_1030']:
            saida.write('|' + '|'.join(item['campos']) + '|\n')
            stats['itens'] += 1

        for lanc in nota['lanc_1300']:
            saida.write('|' + '|'.join(lanc) + '|\n')
            stats['lancamentos'] += 1

    saida.write('|9999|\n')

    if nao_mapeados:
        log.append(
            f"AVISO: {len(nao_mapeados)} CFOP(s) não encontrado(s) na tabela "
            f"(acumulador 9999): {', '.join(sorted(nao_mapeados))}"
        )

    log.append(
        f"Geração concluída — "
        f"Notas={stats['notas']} | "
        f"Entradas={stats['nf_entrada']} | "
        f"Saídas={stats['nf_saida']} | "
        f"Itens={stats['itens']} | "
        f"Lançamentos={stats['lancamentos']} | "
        f"Erros={stats['erros']}"
    )
    return saida.getvalue(), stats


# ==============================
# RELATÓRIO DE ORDENAÇÃO
# ==============================
def gerar_relatorio_ordenacao(notas: list) -> pd.DataFrame:
    linhas = []
    for pos, nota in enumerate(notas, start=1):
        linhas.append({
            'Posição':    pos,
            'Num NF':     nota['num_nf'],
            'CFOP NF':    nota['cfop_nf'],
            'Acumulador': nota['acum_nf'],
            'Dt Emissão': nota['dt_emissao'],
            'Dt Entrada': nota['dt_entrada'],
            'Vl Total':   nota['vl_total'],
            'Qtd Itens':  len(nota['itens_1030']),
            'Qtd Lanç':   len(nota['lanc_1300']),
            'Linha Orig': nota['linha_orig'],
            'Tipo':       'Entrada' if nota['cfop_nf'][:1] in ('1','2','3') else 'Saída',
        })
        for item in nota['itens_1030']:
            linhas.append({
                'Posição':    f"  └ Item {item['num_item']}",
                'Num NF':     nota['num_nf'],
                'CFOP NF':    item['cfop'],
                'Acumulador': item['acum'],
                'Dt Emissão': '',
                'Dt Entrada': '',
                'Vl Total':   _campo(item['campos'], IDX_1030_VL_ITEM),
                'Qtd Itens':  '',
                'Qtd Lanç':   '',
                'Linha Orig': item['linha_orig'],
                'Tipo':       'Item',
            })
    return pd.DataFrame(linhas)


# ==============================
# INTERFACE STREAMLIT
# ==============================
def main():
    st.set_page_config(
        page_title="Domínio Sistemas | Thomson Reuters",
        page_icon="🟠",
        layout="wide",
        initial_sidebar_state="expanded",
    )
    apply_tr_theme()

    # ── Cabeçalho ───────────────────────────────────────────────────────
    st.markdown(
        f"""
        <div style="background:#444444; padding:24px 28px 18px 28px; border-radius:8px;
                    border-top:6px solid #FF8000; margin-bottom:28px;">
            <h2 style="color:#FF8000; margin:0; font-family:'Segoe UI',Arial,sans-serif;">
                📄 Conversor / Ordenador Domínio Sistemas &nbsp;|&nbsp; {VERSAO}
            </h2>
            <p style="color:#DDDDDD; margin:6px 0 0 0; font-family:'Segoe UI',Arial,sans-serif;">
                Faça upload do arquivo Domínio para extrair os CFOPs e gerar a planilha
                de acumuladores, ou processe diretamente com uma tabela já preenchida.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    )

    # ── Sidebar ─────────────────────────────────────────────────────────
    with st.sidebar:
        st.markdown("### ℹ Sobre")
        st.markdown(f"**Versão:** {VERSAO}")
        st.markdown("**Thomson Reuters**")
        st.markdown("**Domínio Sistemas**")
        st.markdown("---")
        st.markdown("### 📋 Registros Suportados")
        st.markdown(
            "- **0000** Cabeçalho\n"
            "- **1000** Nota Fiscal\n"
            "- **1020** Totais da NF\n"
            "- **1030** Itens da NF\n"
            "- **1300** Lançamento Contábil\n"
            "- **9999** Encerramento\n"
        )
        st.markdown("---")
        st.markdown("### 📑 Fluxo recomendado")
        st.markdown(
            "1. Upload do arquivo `.txt`\n"
            "2. **Extrair CFOPs → baixar XLSX**\n"
            "3. Preencher coluna `ACUMULADOR`\n"
            "4. Fazer upload do XLSX preenchido\n"
            "5. **Processar** e baixar saída\n"
        )
        st.markdown("---")
        st.markdown("### ⚙ Ordenação")
        st.markdown("Data Emissão → Nº NF → Nº Item")
        st.markdown("### ⚙ Encoding de saída")
        st.markdown("**ANSI (Latin-1)**")

    # ── Instruções ──────────────────────────────────────────────────────
    with st.expander("📖 **Instruções de Uso** — clique para expandir", expanded=False):
        st.markdown(
            """
            <div class="instrucoes-box">

            <h4>🔹 Etapa 1 — Extrair CFOPs do arquivo</h4>
            <p>Faça o upload do arquivo <code>.txt</code> do Domínio Sistemas e clique em
            <b>🔍 Extrair CFOPs e Gerar Planilha</b>. O sistema identificará todos os CFOPs
            presentes e gerará um arquivo <b>XLSX no tema Thomson Reuters</b> pronto para
            preenchimento.</p>

            <h4>🔹 Etapa 2 — Preencher a planilha de acumuladores</h4>
            <p>Abra o XLSX baixado e preencha a coluna <b>ACUMULADOR</b> para cada CFOP
            conforme a parametrização do seu Domínio Sistemas. Salve o arquivo.</p>

            <h4>🔹 Etapa 3 — Processar o arquivo</h4>
            <p>Faça o upload da planilha preenchida no campo <b>Tabela de Acumuladores</b>
            e clique em <b>▶ Processar Arquivo</b>. O sistema irá ordenar as notas e
            gerar o arquivo final em <b>ANSI (Latin-1)</b>.</p>

            <hr>

            <h4>⚠ Observações</h4>
            <ul>
                <li>CFOPs sem acumulador preenchido receberão <b>9999</b> e serão listados no log.</li>
                <li>A ordenação aplicada é: <b>Data Emissão → Nº NF → Nº Item</b>.</li>
                <li>Registros 1020, 1030 e 1300 sem 1000 pai são ignorados com aviso no log.</li>
            </ul>

            </div>
            """,
            unsafe_allow_html=True,
        )

    st.markdown("---")

    # ── Session state ────────────────────────────────────────────────────
    defaults = {
        "log_conv":           [f"Aplicação pronta. Versão: {VERSAO}"],
        "resultado_conv":     None,
        "nome_saida":         "saida_dominio.txt",
        "stats_conv":         None,
        "df_ordenacao":       None,
        "tabela_acum_ok":     False,
        "tabela_acum_info":   "",
        "xlsx_acum_bytes":    None,
        "xlsx_acum_nome":     "acumuladores.xlsx",
        "cfops_extraidos":    None,
    }
    for k, v in defaults.items():
        if k not in st.session_state:
            st.session_state[k] = v

    # ════════════════════════════════════════════════════════════════════
    # ETAPA 1 — UPLOAD DO ARQUIVO E EXTRAÇÃO DE CFOPs
    # ════════════════════════════════════════════════════════════════════
    st.markdown("### 🔍 Etapa 1 — Extrair CFOPs do arquivo")

    uploaded_file = st.file_uploader(
        "📂 Arquivo Domínio de origem (.txt)",
        type=["txt"],
        help="Arquivo .txt exportado pelo Domínio Sistemas",
        key="upload_dominio",
    )

    col_e1, col_e2 = st.columns([1, 1])

    with col_e1:
        extrair = st.button(
            "🔍 Extrair CFOPs e Gerar Planilha",
            disabled=(uploaded_file is None),
            use_container_width=True,
            type="primary",
        )
    with col_e2:
        # Download da planilha gerada — só aparece depois da extração
        if st.session_state.xlsx_acum_bytes is not None:
            st.download_button(
                label="⬇ Baixar Planilha de Acumuladores (.xlsx)",
                data=st.session_state.xlsx_acum_bytes,
                file_name=st.session_state.xlsx_acum_nome,
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
                type="primary",
            )

    if extrair and uploaded_file is not None:
        st.session_state.log_conv        = ["Extraindo CFOPs do arquivo..."]
        st.session_state.xlsx_acum_bytes = None
        st.session_state.cfops_extraidos = None

        try:
            raw     = uploaded_file.read()
            content = decode_arquivo(raw)
            parsed  = parse_dominio(content)

            cfops_dict = extrair_cfops_do_arquivo(parsed)

            if not cfops_dict:
                st.session_state.log_conv.append(
                    "AVISO: Nenhum CFOP encontrado nos registros 1000 e 1030."
                )
            else:
                xlsx_bytes = gerar_xlsx_acumuladores_tr(cfops_dict)
                nome_base  = uploaded_file.name.replace('.txt', '')
                nome_xlsx  = f"{nome_base}_acumuladores.xlsx"

                st.session_state.xlsx_acum_bytes = xlsx_bytes
                st.session_state.xlsx_acum_nome  = nome_xlsx
                st.session_state.cfops_extraidos = cfops_dict
                st.session_state.log_conv.append(
                    f"✔ {len(cfops_dict)} CFOP(s) único(s) extraído(s) — "
                    f"planilha gerada: {nome_xlsx}"
                )
                # Guarda o parsed para reuso no processamento
                st.session_state["_parsed_cache"]  = parsed
                st.session_state["_arquivo_nome"]  = uploaded_file.name

        except Exception:
            st.session_state.log_conv.append("ERRO FATAL na extração de CFOPs.")
            st.session_state.log_conv.append(traceback.format_exc())

        st.rerun()

    # Exibe resumo dos CFOPs extraídos
    if st.session_state.cfops_extraidos:
        cfops_dict = st.session_state.cfops_extraidos
        n_ent = sum(1 for v in cfops_dict.values() if v['tipo_operacao'] == 'Entrada')
        n_sai = sum(1 for v in cfops_dict.values() if v['tipo_operacao'] == 'Saída')

        col_m1, col_m2, col_m3 = st.columns(3)
        col_m1.metric("CFOPs únicos",  len(cfops_dict))
        col_m2.metric("Entradas",      n_ent)
        col_m3.metric("Saídas",        n_sai)

        with st.expander("📋 CFOPs identificados no arquivo", expanded=False):
            rows = []
            for cfop, info in sorted(cfops_dict.items()):
                rows.append({
                    'CFOP':          cfop,
                    'Descrição':     DESCRICOES_CFOP.get(cfop, '—'),
                    'Tipo':          info['tipo_operacao'],
                    'Ocorrências':   info['ocorrencias'],
                    'Registros':     ', '.join(sorted(info['registros'])),
                })
            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)

    st.markdown("---")

    # ════════════════════════════════════════════════════════════════════
    # ETAPA 2 — UPLOAD DA TABELA PREENCHIDA E PROCESSAMENTO
    # ════════════════════════════════════════════════════════════════════
    st.markdown("### ▶ Etapa 2 — Processar com a tabela de acumuladores preenchida")

    col_up1, col_up2 = st.columns([1, 1])

    with col_up1:
        arquivo_acum = st.file_uploader(
            "📂 Tabela de Acumuladores preenchida (.xlsx ou .csv)",
            type=["xlsx", "xls", "csv"],
            help="Planilha com colunas CFOP e ACUMULADOR preenchidos",
            key="upload_acum",
        )

    with col_up2:
        arquivo_sped_2 = st.file_uploader(
            "📂 Arquivo Domínio de origem (.txt) — se diferente do da Etapa 1",
            type=["txt"],
            help="Pode ser o mesmo arquivo da Etapa 1 ou um novo",
            key="upload_dominio_2",
        )

    # Feedback da tabela de acumuladores
    if arquivo_acum is not None:
        log_temp  = []
        raw_acum  = arquivo_acum.read()
        tab_prev  = carregar_acumuladores(raw_acum, arquivo_acum.name, log_temp)
        arquivo_acum.seek(0)
        if tab_prev is not None:
            st.success(
                f"✅ Tabela válida — **{len(tab_prev)} CFOPs** com acumulador preenchido."
            )
            st.session_state.tabela_acum_ok   = True
            st.session_state.tabela_acum_info = f"{len(tab_prev)} CFOPs mapeados."
        else:
            for msg in log_temp:
                st.error(msg)
            st.session_state.tabela_acum_ok   = False
            st.session_state.tabela_acum_info = ""
    else:
        st.info("⬆ Faça o upload da tabela de acumuladores preenchida para processar.")
        st.session_state.tabela_acum_ok = False

    # Determina qual arquivo usar para o processamento
    tem_arquivo_para_processar = (
        arquivo_sped_2 is not None or
        st.session_state.get("_parsed_cache") is not None
    )

    pode_processar = (
        st.session_state.tabela_acum_ok and
        tem_arquivo_para_processar
    )

    col1, col2 = st.columns([1, 1])
    with col1:
        processar = st.button(
            "▶ Processar Arquivo",
            disabled=not pode_processar,
            use_container_width=True,
            type="primary",
        )
    with col2:
        limpar = st.button("🗑 Limpar Tudo", use_container_width=True)

    if limpar:
        for k in list(st.session_state.keys()):
            del st.session_state[k]
        st.rerun()

    # ── Processamento ────────────────────────────────────────────────────
    if processar and pode_processar:
        st.session_state.log_conv      = ["Iniciando processamento..."]
        st.session_state.resultado_conv = None
        st.session_state.stats_conv     = None
        st.session_state.df_ordenacao   = None

        try:
            # Carrega tabela de acumuladores
            arquivo_acum.seek(0)
            tabela_acum = carregar_acumuladores(
                arquivo_acum.read(),
                arquivo_acum.name,
                st.session_state.log_conv,
            )
            if tabela_acum is None:
                st.session_state.log_conv.append("ERRO: Tabela inválida. Abortando.")
                st.rerun()

            # Usa o arquivo da Etapa 2 se fornecido, senão usa o cache da Etapa 1
            if arquivo_sped_2 is not None:
                raw     = arquivo_sped_2.read()
                content = decode_arquivo(raw)
                parsed  = parse_dominio(content)
                nome_arq = arquivo_sped_2.name
                st.session_state.log_conv.append(
                    f"Usando arquivo da Etapa 2: {nome_arq}"
                )
            else:
                parsed   = st.session_state["_parsed_cache"]
                nome_arq = st.session_state.get("_arquivo_nome", "arquivo.txt")
                st.session_state.log_conv.append(
                    f"Usando arquivo da Etapa 1: {nome_arq}"
                )

            tipos_enc = list(parsed['por_tipo'].keys())
            st.session_state.log_conv.append(
                f"Tipos de registro: {', '.join(tipos_enc)}"
            )

            # Gera saída
            resultado_txt, stats = gerar_saida(
                parsed, tabela_acum, st.session_state.log_conv
            )

            # Relatório de ordenação
            nao_map_tmp = set()
            notas_ord   = extrair_notas_com_itens(parsed, tabela_acum, nao_map_tmp, [])
            df_ord      = gerar_relatorio_ordenacao(notas_ord)

            # Codifica em ANSI
            resultado_bytes = encode_ansi_seguro(
                resultado_txt, st.session_state.log_conv
            )

            st.session_state.resultado_conv = resultado_bytes
            st.session_state.stats_conv     = stats
            st.session_state.df_ordenacao   = df_ord
            st.session_state.nome_saida     = nome_arq.replace('.txt', '_processado.txt')

        except Exception:
            st.session_state.log_conv.append("ERRO FATAL durante o processamento.")
            st.session_state.log_conv.append(traceback.format_exc())

        st.rerun()

    # ── Resultado ────────────────────────────────────────────────────────
    if st.session_state.resultado_conv is not None:
        st.success("✅ Arquivo processado com sucesso!")

        stats = st.session_state.stats_conv or {}
        st.markdown("#### 📊 Estatísticas")
        col1, col2, col3 = st.columns(3)
        col1.metric("Total de Notas", stats.get('notas',       0))
        col2.metric("NFs Entrada",    stats.get('nf_entrada',  0))
        col3.metric("NFs Saída",      stats.get('nf_saida',    0))

        col4, col5, col6 = st.columns(3)
        col4.metric("Itens",          stats.get('itens',       0))
        col5.metric("Lançamentos",    stats.get('lancamentos', 0))
        col6.metric("Erros",          stats.get('erros',       0))

        st.markdown("---")

        if st.session_state.df_ordenacao is not None:
            with st.expander(
                "📋 Verificação de Ordenação — clique para expandir",
                expanded=True,
            ):
                st.markdown(
                    "Notas na **ordem exata** gravada no arquivo de saída."
                )
                st.dataframe(
                    st.session_state.df_ordenacao,
                    use_container_width=True,
                    hide_index=True,
                )

        with st.expander("👁️ Prévia do arquivo gerado (primeiras 60 linhas)"):
            preview = '\n'.join(
                st.session_state.resultado_conv
                .decode('latin-1', errors='replace')
                .splitlines()[:60]
            )
            st.code(preview, language='text')

        st.download_button(
            label="⬇ Baixar Arquivo Processado",
            data=st.session_state.resultado_conv,
            file_name=st.session_state.nome_saida,
            mime="text/plain",
            use_container_width=True,
            type="primary",
        )

    # ── Log ──────────────────────────────────────────────────────────────
    st.markdown("**Log de processamento**")
    log_texto = "\n".join(st.session_state.log_conv)
    tem_erro  = any(str(l).startswith("ERRO") for l in st.session_state.log_conv)
    cor_borda = "#D32F2F" if tem_erro else "#388E3C"

    st.markdown(
        f"""
        <div style="background:#FCFCFC; border:1px solid {cor_borda};
                    border-radius:6px; padding:14px;
                    font-family:Consolas,monospace; font-size:13px;
                    white-space:pre-wrap; max-height:340px;
                    overflow-y:auto; color:#1F1F1F;">
{log_texto}
        </div>
        """,
        unsafe_allow_html=True,
    )

    st.markdown("---")
    st.caption(
        "Conversor / Ordenador Domínio Sistemas | "
        "Thomson Reuters | Desenvolvido com Python + Streamlit"
    )


if __name__ == "__main__":
    main()
